const treeData = [
  {name:"work",type:"folder",children:[
    ...
    {name:"api-flow.tsx",type:"tsx",page:"api"},
    {name:"tiffome.tsx",type:"tsx",page:"tiffome"},
    {name:"nexus.tsx",type:"tsx",page:"nexus"}
  ]},
...
const projects = {
commit:{file:"commit.tsx",title:"Commit — Focus Timer",kicker:"React Native · Firebase · Payments",intro:"A focused mobile productivity experience built around timed sessions, progress feedback, and simple interactions.",desc:"The product is designed around one clear job: start a focused session and make the active state easy to understand. The portfolio version shows the product thinking without inventing production metrics.",engineering:"State management for session tracking, local persistence with AsyncStorage, payment integration with Razorpay, real-time timer updates with accurate elapsed time.",challenges:"Keeping the UI responsive during rapid state updates, managing timer accuracy across app states, clean abstraction for session logic.",stack:["React Native","Expo","Firebase","Razorpay"],visual:"phone"},
api:{file:"api-flow.tsx",title:"API Flow",kicker:"Full-Stack · Node.js · MySQL",intro:"A workflow-builder concept for connecting API requests into repeatable flows with a developer-first interface.",desc:"The concept explores API authentication, flow steps, persistence, and execution history in one workspace. It is also a useful example of the frontend-to-backend work behind the portfolio.",engineering:"Visual flow builder with drag-and-drop nodes, state synchronization between client and server, request/response handling, execution history tracking and replay.",challenges:"Managing complex state for flow definitions, handling async operations in sequence, real-time updates without over-fetching, designing intuitive API node configuration.",stack:["React","TypeScript","Node.js","Express","MySQL"],visual:"flow"},
tiffome:{file:"tiffome.tsx",title:"Tiffome",kicker:"Web · React · Product UI",intro:"A food-ordering web concept focused on discovery, visual hierarchy, and a short path to action.",desc:"The interface is intentionally product-focused: fewer competing actions, clear pricing, responsive layouts, and reusable UI patterns.",engineering:"Component-based architecture with reusable card patterns, responsive grid layout system, lazy-loaded images, optimized asset pipeline.",challenges:"Designing visual hierarchy that guides without overwhelming, creating mobile-first responsive layouts, maintaining performance with product imagery.",stack:["React","JavaScript","Responsive UI"],visual:"meal"},
nexus:{file:"nexus.tsx",title:"Nexus Analytics",kicker:"Dashboard · React · Data UI",intro:"A dashboard concept showing how dense product data can remain readable and actionable.",desc:"The visual system uses hierarchy, compact metrics, and progressive detail instead of turning the screen into a wall of charts.",engineering:"Data aggregation layer, efficient chart rendering, metric calculations, responsive grid dashboard layout, data caching strategy.",challenges:"Presenting dense data without cognitive overload, optimizing chart re-renders on data updates, maintaining readability at multiple breakpoints.",stack:["React","TypeScript","Data UI"],visual:"dashboard"}
};
...
function show(page){
current=page; document.getElementById("content").innerHTML=pageContent(page);
const tabs=document.getElementById("tabs"); 
if(![...tabs.children].some(x=>x.dataset.page===page)){
  const t=document.createElement("div");t.className="tab active";t.dataset.page=page;t.innerHTML=`<span>${label(page)}</span><span class="close">×</span>`;
  t.querySelector(".close").onclick=(e)=>{e.stopPropagation();t.remove();show("home")};
  tabs.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));tabs.appendChild(t);
}else tabs.querySelectorAll(".tab").forEach(x=>x.classList.toggle("active",x.dataset.page===page));
renderTree();
...
const files=[
  ...
  {label:"commit.tsx",page:"commit"},
  {label:"api-flow.tsx",page:"api"},
  {label:"tiffome.tsx",page:"tiffome"},
  {label:"nexus.tsx",page:"nexus"},
